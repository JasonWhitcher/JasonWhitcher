...
function dg_get_score_total(playerid){
	...
	for ( var hole_number = 1; hole_number <= holes_played; hole_number++ ){
		if ( parseInt(dg_scorecard[0][hole_number]) > 0 ){ // Check to see if the hole par is not 0 (hole not played). 
			...
			//console.log('score ' + playerid + ':' + total_score);
		}
	}
	//console.log('player' + playerid + ':score' + total_score);
	return total_score;
}
...
function dg_get_game_par(playerid){
	...
	//console.log('holes played:' + holes_played);
	for ( var hole_number = 1; hole_number <= holes_played; hole_number++ ){
	...
	//console.log('total game par:' + total_par);
	return total_par;
}
...
function dg_get_relative_par(playerid){
	...
	//console.log('relative par:' + playerid + ':' + relative_par);
	return relative_par;
}
...
function dg_show_modal(current_context){
	...
	//console.log('Context from show modal:' + dg_current_context_modal);
};
/**
...
function dg_show_page(current_context){
	...
	//console.log('Context from show page1:' + dg_current_context_page);
	if(dg_current_context_page != 'closed'){
		dg_hide_page(dg_current_context_page);
	}
	document.getElementById(current_context).classList.add('dg-show');
	dg_current_context_page = current_context;
	//console.log('Context from show page:' + dg_current_context_page);

	// Check to see if the current hole's par is set.
	//console.log('scorecard hole par:' + dg_scorecard[0][dg_current_hole]);
	if ( dg_scorecard[0][dg_current_hole] <= 0 || dg_scorecard[0][dg_current_hole] == undefined ) {
		//console.log('context page(show page):' + dg_current_context_page);
		dg_hide_page(dg_current_context_page);
		dg_show_modal('dg-set-par-container');
		dg_display_set_par();
	}
	...
	for ( var playerid = 1; playerid <= dg_scorecard.length - 1; playerid++ ){
		...
		console.log('dg_scorecard[playerid][dg_current_hole]:' + dg_scorecard[playerid][dg_current_hole]);
		console.log('playerid:' + playerid);
		console.log('dg_current_hole:' + dg_current_hole);
		console.log('dg_main_scoring_current_hole_score:' + dg_main_scoring_current_hole_score);
...
function dg_hide_page(current_context){
	//console.log('context page(hide page):' + dg_current_context_page);
	document.getElementById(current_context).classList.remove('dg-show');
}
/**
...
function dg_main_scoring_change_hole(dg_button){
	...
	for (x = 0; x < dg_elements_main_scoring_line.length; x++){
		...
		//console.log('scorecard:' + dg_scorecard[playerid][dg_current_hole]);
	}
...
function dg_start_game(){
	...
	for (var x = 0; x < dg_scorecard.length; x++) {
		dg_scorecard[x] = [];
		console.log('am i here?' + dg_scorecard[x][0]);
	
	}
	//dg_scorecard = new Array();
	//dg_scorecard = [[]]; // Reset the scorecard[][] array.
...
function dg_add_player(){
	...
	for (x=0; x<dg_temporary_player_list.length; x++){
		//console.log('player' + x + ':' + dg_temporary_player_list[x]);
	}
}
...
/**
  ...
  @param	none
  @return	nonedg_init_scorecard(player_list)
*/
function dg_enter_starting_hole(){
	...
	//console.log('starting hole:' + dg_current_hole);
...
function dg_display_set_par(){
	...
	//console.log('dg_display_set_par() current hole:' + dg_current_hole);
...
function dg_set_par(){
	...
	//console.log('dg_set_par() current hole:' + dg_current_hole);
	...
	for (playerid = 0; playerid < number_of_players; playerid++){
		for (hole = 0; hole <= dg_current_hole; hole++) {
			//console.log('dg_scorecard[' + playerid + '][' + hole + ']=' + dg_scorecard[playerid][hole]);
		}
	}
	dg_hide_modal(dg_current_context_modal);
	dg_show_page(dg_current_context_page);
...
function dg_display_scorecard_page(){
	...
	for (var playerid = 1; playerid <= dg_scorecard.length - 1; playerid++) { // -1 to account for index 0 (par).
		console.log('playeridA:' + playerid);
